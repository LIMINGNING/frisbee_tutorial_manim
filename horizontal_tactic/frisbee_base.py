...
class FrisbeeBaseScene(MovingCameraScene):
    ...
    def move_camera_to_player(self, target_player, vertical_only=True, target_camera_pos=None, 
                         run_time=1.0, zoom_level=None, offset=None, rate_func=smooth):
        """
        将相机视角移动到指定玩家位置
        
        参数:
        target_player - 目标玩家对象
        vertical_only - 是否只调整垂直位置(保持X坐标不变)，默认True
        target_camera_pos - 自定义相机目标位置(可选)，如果提供则忽略其他位置参数
        run_time - 动画时长，默认1.0
        zoom_level - 相机缩放级别(可选)
        offset - 相对于玩家位置的偏移量(可选)，例如np.array([0, 1, 0])让相机在玩家上方1单位
        rate_func - 动画速率函数，默认smooth
        """
        # 获取玩家位置
        player_pos = target_player.get_center()
        ...
        if target_camera_pos is not None:
            ...
            if vertical_only:
                ...
                # 完全移动到玩家位置
                camera_target = player_pos.copy()
                
            # 应用偏移量(如果提供)
            if offset is not None:
                camera_target = camera_target + offset
        ...
        if zoom_level is not None:
            current_height = self.camera.frame.height
            camera_anim = self.camera.frame.animate.set_height(zoom_level)
        ...
        return camera_target  # 返回最终相机位置，以便后续使用
        pass
